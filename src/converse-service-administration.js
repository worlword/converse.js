...
const { $iq, Backbone, Strophe, _ } = converse.env;
    ...
    initialize () {
        ...
        _converse.ServiceAdministration = Backbone.Model.extend({
            async discoverSupport () {
                const supported = await _converse.api.disco.supports('http://jabber.org/protocol/commands', _converse.bare_jid);
                if (supported) {
                    const stanza = $iq({
                    ...
                    _converse.connection.sendIQ(stanza, result => {
                        const commands = converse.env.sizzle('item[node^="http://jabber.org/protocol/admin"]', result);
        ...
        _converse.ServiceAdminCommandItemView = Backbone.NativeView.extend({
            ...
            openServiceAdminChat () {
                ...
                _converse.connection.sendIQ(stanza, result => {
                    ...
                    const session_id = converse.env.sizzle('command', this.server_response).pop().getAttribute('sessionid');
                    const title = converse.env.sizzle('title', this.server_response).pop();
                    const fields = converse.env.sizzle('field', this.server_response);
                    const form_type_value = converse.env.sizzle('field[var="FORM_TYPE"] > value', this.server_response).pop().innerHTML;
            ...
            getValueOfFieldElement (field) {
                const values = converse.env.sizzle('value', field);
                let string_value = '';
        ...
        _converse.ServiceAdminBoxView = _converse.ChatBoxView.extend({
            ...
            renderTextMultiField (field) {
                ...
                const text_area = converse.env.sizzle('textarea[name="' + field['var'] + '"]', chat_body).pop();
            ...
            close (ev) {
                ...
                // there is still a problem here, that the following line causes an infinite loop.
            ...
            sendMessageToServer (ev) {
                ...
                _converse.connection.sendIQ(stanza, success => {
                    alert(__("The command was executed successfully."));
                }, error => {
                    const error_text = converse.env.sizzle('error > text', error).pop().innerHTML;
